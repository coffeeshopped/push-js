...
function PushPresetSaver(patcher, options) {
...
PushPresetSaver.prototype = Object.create(PushWidget.prototype)

PushPresetSaver.prototype.buttonSelected = function(row, col) {
  var slot = row * 8 + col + 1
  this.pattrstorage.message("store", slot)
  this.pattrstorage.message("writeagain") // save to disk
  // get slot list to update display
...
PushPresetSaver.prototype.link = function(push) {
...
PushPresetSaver.prototype.slotList = function(slots) {
  if (!this.push) { return }
  ...
  for (var col=0; col<8; ++col) {
    for (var row=0; row<8; ++row) {
      ...
      button.displayValue(open ? 40 : 60);
    }
  }  
}

PushPresetSaver.prototype.current = function(current) {
  if (!this.push) { return }
  if (current < 0) { return }
  ...
  var button = this.selectButtons[col][row];
  button.displayValue(65);
}

PushPresetSaver.prototype.unlink = function() {
  for (var col=0; col<8; ++col) {
    for (var row=0; row<8; ++row) {
...
this.exports.PushPresetSaver = PushPresetSaver
